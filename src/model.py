...
class ConvText(object):
    ...
    """
    def __init__(self, seq_length, num_labels, embedding_weights):
    ...
    def build(self):
        ...
        self.target_tags = tf.placeholder(tf.float32, (None, self.num_labels))


        # Embedding layer
        E = tf.get_variable('E', 
                            initializer=tf.constant(self.embedding_weights),
                            trainable=True)
        ...
        # Dense layer 4
        flatten_4 = tf.layers.Flatten(conv_3)
        dense_4 = tf.layers.dense(flatten_4,
                                units=250,
                                activation=tf.nn.relu)
